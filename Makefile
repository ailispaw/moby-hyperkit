...
status:
	@if [ ! -f vm/.mac_address ]; then \
		echo "moby-hyperkit: stopped"; \
		exit 1; \
	else \
		if ping -c 1 -t 1 $(IP_ADDR) >/dev/null 2>&1; then \
			echo "moby-hyperkit: running on $(IP_ADDR)"; \
		else \
			echo "moby-hyperkit: starting"; \
			exit 1; \
		fi; \
	fi >&2;

version: | status
	@make ssh cat /etc/os-release 2>/dev/null | sed -n 's/^VERSION_ID=\(.*\)$$/v\1/p'

.PHONY: run up mac ip ssh halt env status version
